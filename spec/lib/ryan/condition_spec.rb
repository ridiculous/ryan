...
describe Ryan::Condition do
  ...
  describe '#full_statement' do
    it 'returns the full statement as a string' do
      expect(subject.full_statement).to eq 'return [] unless message[:verification_code_id]'
    end
  end
  ...
  describe '#statement' do
    context 'when a single line expression with unless modifier' do
      it 'returns an accurate summary of the condition' do
        expect(subject.statement).to eq 'unless message[:verification_code_id]'
      end
    end
  end
  ...
  describe '#if_text' do
    context 'when a single line expression with unless modifier' do
      it 'returns the correct return value' do
        expect(subject.if_text).to eq 'returns []'
      end
    end
    ...
    context 'when the code return value is nil' do
      ...
      it 'says it returns nil' do
        expect(subject.if_text).to eq 'returns nil'
      end
    end
...
'
      ...
      it 'returns the correct value' do
        expect(subject.if_text).to eq %Q(returns [\"\#{file}.rb\"])
      end
    end
    ...
    context 'multiline if/elsif/else' do
      ...
      it 'returns the correct return value' do
        expect(subject.if_text).to eq "returns report.perform"
      end
    end
    ...
    context 'when the statement contains a begin/end block' do
      ...
      it 'returns the last line of the begin' do
        expect(subject.if_text).to eq 'returns user.cards!'
      end
    end
    ...
    context 'when given a condition with only a nested condition' do
      ...
      it 'returns an empty string' do
        expect(subject.if_text).to eq ''
      end
    end
  end

  describe '#else_text' do
    context 'when given an if/else statement' do
      ...
      it 'returns the return value of the else statement' do
        expect(subject.else_text).to eq 'returns fallback.call(env)'
      end
    end

    context 'when if statement with no else' do
      it 'returns an empty string' do
        expect(subject.else_text).to eq ''
      end
    end
    ...
    context 'when given a block for the else statement' do
      ...
      it 'returns the full statement (broken)' do
        expect(subject.else_text).to eq "returns @current_user ||= if defined? @current_user then\n  @current_user\nelse\n  begin\n  begin\n    User.find_by!(:auth_token => cookies[:auth_token]) if cookies[:auth_token]\n  rescue ActiveRecord::RecordNotFound\n    Rails.logger.warn(\"Could not find user w/ token \\\"\#{cookies[:auth_token]}\\\"\")\n    nil\n  end\n  end\nend"
      end
    end
  end
  ...
  describe '#parts' do
    ...
    context 'when if/elsif/else' do
      ...
      it 'returns the parts of the condition' do
        ...
        expect(subject.parts.map(&:statement)).to eq ['(dude == "1")', '(you == "bar")']
        expect(subject.parts.map(&:if_text)).to eq ['returns call_foo(dude)', 'returns call_bar(you)']
        expect(subject.parts.map(&:else_text)).to eq ['returns call_foo(you)', 'returns "wut"']
      end
    end
  end
end
